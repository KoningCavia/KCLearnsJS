...
const calcDisplaySummary = function (acc) {
  ...
  const interest = acc.movements
    ...
    .filter((int, i, arr) => {
      // add filter that only adds interest thats higher than 1
      return int >= 1;
    })
...
btnTransfer.addEventListener('click', function (e) {
  ...
  ); // We retrieve the account which has the username which is the same as the name entered in the text field.
...
btnClose.addEventListener('click', function (e) {
  ...
  if (
    currentAccount.username === closeAccount &&
    +currentAccount.pin === +closePin
  ) {
    const index = accounts.findIndex(
      (
        acc // this works exactly like the find method. except it
...
const randomInt = (min, max) =>
  Math.trunc(Math.random() * (max - min) + 1) + min; //around t=6:20 it is explained mathematically why this function results in a random value between a given min and max value (rather than zero and some value).
...
console.log(
  navigator.language,
  'BROSWER: ',
  new Intl.NumberFormat(navigator.language, options).format(numb)
);

console.log(
  'NEDERLANDSSSSSSSS: ',
  new Intl.NumberFormat('nl-NL', options).format(numb)
);

// 181 TIMERS setTimeout and setInterval

console.log('----- 181 TIMERS -----');

// setTimeout works with a callback function. The timer is set as a second argument.
setTimeout(() => console.log('Here is your pizza'), 3000);
//JS sees the above method and registers that it should be called after 3 seconds. it will then
//immediately go to the next line. so the log below actually gets processed quicker.

// now adding arguments to a settimeout function is a bit tricky, because we dont call the callback function ourselves.
...
//using an array and the spread operator: seems IMPORTANT
...
//below slears the timermethod if spinach is an ingredient (which it is)
if (ingredients.includes('spinach')) clearTimeout(pizzaTimer);

//setInterval
// CLOCK
// every second returns the time.
...
const clockSeconds = `clock.getSeconds()`.padStart;
